...
export class AuthorizationGuard implements CanActivate {
    ...
    convertPermission(permission: PermissionType): PermissionType[] {
        ...
        if (action === PERMISSION_ACTION.MANAGE_ALL) {
            return [
                `${PERMISSION_ACTION.CREATE}_${resource}`,
                `${PERMISSION_ACTION.READ}_${resource}`,
                `${PERMISSION_ACTION.UPDATE}_${resource}`,
                `${PERMISSION_ACTION.DELETE}_${resource}`,
                `${PERMISSION_ACTION.LOGIN}_${resource}`,
                `${PERMISSION_ACTION.READ_ALL}_${resource}`,
                `${PERMISSION_ACTION.UPDATE_STATUS}_${resource}`,
            ];
